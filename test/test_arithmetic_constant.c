...
UT_FUNC_GEN(test_arithmetic_constant) {
    UT_GROUP_BEG(constant) {
        ...
        UT_GROUP_BEG(one) {
            ...
            UT_ASSERT_FLT_EQ(f32_c(1.0), one(float));
        ...
        UT_GROUP_BEG(float_point) {
            ...
            UT_ASSERT_TYP_NQ(             1, pos(zero(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), pos(zero(fmax_t)));

            UT_ASSERT_FLT_EQ(       fmax_c(-0.0), neg(zero(fmax_t)));
            UT_ASSERT_TYP_NQ(             1, neg(zero(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), neg(zero(fmax_t)));

            UT_ASSERT_FLT_EQ(  fmax_c(+INFINITY), pos(inf(fmax_t)));
            UT_ASSERT_TYP_NQ(             1, pos(inf(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), pos(inf(fmax_t)));

            UT_ASSERT_FLT_EQ(  fmax_c(-INFINITY), neg(inf(fmax_t)));
            UT_ASSERT_TYP_NQ(             1, neg(inf(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), neg(inf(fmax_t)));

            UT_ASSERT_FLT_EQ(        fmax_c(+NAN), pos(nan(fmax_t)));
            UT_ASSERT_TYP_NQ(             1, pos(nan(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), pos(nan(fmax_t)));

            UT_ASSERT_FLT_EQ(        fmax_c(-NAN), neg(nan(fmax_t)));
            UT_ASSERT_TYP_NQ(             1, neg(nan(fmax_t)));
            UT_ASSERT_TYP_EQ(       fmax_c(1.0), neg(nan(fmax_t)));
        }UT_GROUP_END;
    }UT_GROUP_END;
}
