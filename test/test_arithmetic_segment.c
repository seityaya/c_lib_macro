...
UT_FUNC_GEN(test_arithmetic_segment) {
    UT_GROUP_BEG(segment) {
        ...
        UT_GROUP_BEG(get_rem) {
            UT_ASSERT_NUM_EQ(  0, get_rem(   2));
            UT_ASSERT_NUM_EQ(  0, get_rem(  -2));

            UT_ASSERT_FLT_EQ( 0.1, get_rem( 2.1));
            UT_ASSERT_FLT_EQ(-0.1, get_rem(-2.1));
            UT_ASSERT_FLT_EQ( 0.0, get_rem( 2.0));
            UT_ASSERT_FLT_EQ( 0.0, get_rem(-2.0));
            UT_ASSERT_FLT_EQ( 0.9, get_rem( 1.9));
            UT_ASSERT_FLT_EQ(-0.9, get_rem(-1.9));
            UT_ASSERT_FLT_EQ( 0.1, get_rem( 1.1));
            UT_ASSERT_FLT_EQ(-0.1, get_rem(-1.1));
        }UT_GROUP_END;
